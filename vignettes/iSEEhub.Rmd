...
The selectize input labelled `Show columns:` may be used to add, remove, or reorder columns in the table.
...
This pane is contains information and the users inputs required to load the currently selected dataset in the main about the currently selected dataset in the main `r Biocpkg("iSEE")` app.

The pane is composed of two tabs.
...
As such, users can uses a minimal subset of columns in the table to efficiently browse available datasets, while having a full overview of the currently selected dataset in this pane.
...
Instead, the `Default` option prompts the app to automatically identify all the built-in `r Biocpkg("iSEE")` panel classes that are compatible with the dataset, initialising an app that contains one instance of each of those panels.

The number of panels and data points to draw for large datasets makes the `Default` option a showcase mode for demonstration and for new users, more than an one optimised for a short loading time or featuring specific aspects of the dataset.
...
When selected, the contents of the configuration file are shown in the pane -- including any comment from the authors -- allowing users to review the script if they so wish.
...
<img src="screenshots/dependencies_missing_blocked" width="100%" />
